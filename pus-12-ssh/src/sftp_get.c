...
int main(int argc, char **argv) {
    ...
    /*
     * Zdefiniowanie sciezki do katalogu domint main() {
    ...
    save = fopen("Pobrany_plik", "w+");
    if(!save){
        printf("Wystapil blad przy tworzeniu pliku docelowego\n");
    }
    ...
    if(!handle){
        ...
        fprintf(stdout,"Pobieranie pliku %s\n",dir);
        while(1){
            bytes = libssh2_sftp_read(handle,mem,sizeof(mem));
            if(bytes > 0){
                fwrite(mem, bytes, 1 , save);
                fprintf(stdout, "Pobrano plik\n");
            }
            else{
                break;
            }
