...
#define SOURCE_ADDRESS "192.168.111.150"
...
int main(int argc, char** argv) {
    ...
    tcp_header->th_flags              =       2;
    tcp_header->th_seq                =       ntohl(1);
    tcp_header->th_win                =       20;
    tcp_header->th_off                =       5;
    ...
    for (;;) {
        tcp_header->th_sport = htons((uint16_t)(SOURCE_PORT + i%20));
        tcp_header->th_seq                  =       ntohl(i);
        i++;
        /*
