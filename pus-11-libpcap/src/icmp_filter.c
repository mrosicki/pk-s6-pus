 ...
 * Kompilacja:          $ gcc icmp_filter.c -lpcap -o icmp_filter
 * Uruchamianie:        $ ./icmp_filter <INTERFACE NAME>
 */
...
int main(int argc, char **argv) {
    ...
    struct bpf_program * compiled_filter = malloc(sizeof(struct bpf_program));
    ...
    if(pcap_compile(descriptor, compiled_filter, "icmp[icmptype] == 8 || icmp[icmptype] == 0", 1, 0) == -1) {
        pcap_perror(descriptor, "pcap_compile()");
        exit(EXIT_FAILURE);
    }

    if(pcap_setfilter(descriptor, compiled_filter) == -1) {
